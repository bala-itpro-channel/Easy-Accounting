...
export class MasterFakeBackendInterceptor implements HttpInterceptor {
    ...
    intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
        ...
        const currencies: any[] = JSON.parse(localStorage.getItem('currencies'));
        ...
        return of(null).pipe( mergeMap(() => {
            ...
            if (request.url.match(/\/currency\/\d+$/) && request.method === 'GET') {
                ...
                const matchedcurrency = currencies.filter(currency => currency.id === id);
                const currency = matchedcurrency.length ? matchedcurrency[0] : null;
            ...
            // if (request.url.match(/\/currency\/\d+$/) && request.method === 'POST') {
                // find user by id in users array
                const newId = currencies[currencies.length - 1] ? currencies[currencies.length - 1].id + 1 : 1;
            ...
            if (request.url.endsWith('/currency') && request.method === 'PUT') {
                ...
                const indx = currencies.findIndex(dat => {
                                return dat.id == id;
                            });
                if (indx > -1) {
